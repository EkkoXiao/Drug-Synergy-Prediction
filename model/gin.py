...
class GNN(torch.nn.Module):
    ...
    def forward(self, *argv):
        ...
        if self.cat_grep:
            ...
            # 将 h_graph（每个图的聚合表示）扩展一个维度并与 batch_node 沿着第二个维度（dim=1）拼接。是为了将图的全局表示与每个节点的表示结合起来，增加一个额外的节点维度
            batch_mask = torch.cat([torch.ones((batch_mask.shape[0], 1), dtype=torch.long, device=batch.device), batch_mask], dim=1)
            # 创建一个形状为 [B, 1] 的全1张量，并将其与 batch_mask 沿着第二个维度拼接。是为了更新 batch_mask，以反映 batch_node 中新增的全局图表示
