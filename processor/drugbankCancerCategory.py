...
dataset_name = "drugbankCancerCategory/"
...
category_dict = pd.Series(category_df['Category'].values, index=category_df['Item']).to_dict()
...
train_data = pd.read_csv(data_path + 'drugbankCancer_all.csv')
train_data = np.array(train_data)
i = 0
for j in range(len(train_data)):
    try:
