...
for j in range(len(test_data)): 
    ...
    text = test_data[j][2]
    if test_data[j][5].lower() not in text.lower() or test_data[j][6].lower() not in text.lower() or pd.isna(test_data[j][3]):
...
for j in range(len(train_data)):
    ...
    text = train_data[j][2]
    if train_data[j][5].lower() not in text.lower() or train_data[j][6].lower() not in text.lower() or pd.isna(train_data[j][3]):
        print(f"Skipped text: {text}")
        continue
