...
dataset_name = "drugbankCancer/"

shutil.rmtree(data_path + dataset_name)

invalid_list = []

cancer_drug = pd.read_excel(dict_path + "propertyCancer.xlsx")
...
test_data = pd.read_csv(data_path + 'drugbankCancer_test.csv')
...
for j in range(len(test_data)): 
    ...
    text = test_data[j][2]
    if test_data[j][5].lower() not in text.lower() and test_data[j][6].lower() not in text.lower():
...
train_data = pd.read_csv(data_path + 'drugbankCancer_train.csv')
...
for j in range(len(train_data)):
    ...
    text = train_data[j][2]
    if train_data[j][5].lower() not in text.lower() and train_data[j][6].lower() not in text.lower():
        print(f"Skipped text: {text}")
        continue
    ...
    file.write(text)
    file.close()
    smiles1 = train_data[j][6]
    smiles2 = train_data[j][7]
...
for id in invalid_list:
    if pd.notna(drug_dict[id]) and drug_dict[id] != "Not Available":
        print(id)